...
parser.add_argument('--PNG_DIR', type=str, required=False, default='C:\SYSTEMS\PROG\PYTHON\climate_extremes\gk2a_sst\output\gk2a_ami_le3_sst01m-anom_ko020lc_202606010000.png')
...
# parser.add_argument('--PNG_DIR', type=str, required=False, default='C:\SYSTEMS\PROG\PYTHON\climate_extremes\gk2a_sst\output\gk2a_ami_le3_sst01m-diff_ko020lc_202606010000.png')
...
# ── npy 로드 ─────────────────────────────────────────────────
# base_path = os.path.join(NPY_DIR, f'gk2a_sst_monthly_{BASE_MONTH}.npy')
# comp_path = os.path.join(NPY_DIR, f'gk2a_sst_monthly_{COMP_MONTH}.npy')
#
...
fig = plt.figure(figsize=(2000/300, 2200/300))
...
x, y = m(lon, lat)
#pcm = m.pcolormesh(x, y, anomaly, cmap='bwr', shading='auto',vmin=VMIN, vmax=VMAX)
...
m.drawparallels(np.arange(28,47,4), labels=[1,0,0,0], fontsize=10, fontname='DejaVu Sans', fmt='%d',fontweight='bold')
m.drawmeridians(np.arange(120,136,4), labels=[0,0,0,1], fontsize=10, fontname='DejaVu Sans', fmt='%d',fontweight='bold')

# 2026.05.29 수정
#  컨투어 선 (1°C 간격, 0 라인 강조)
line_levels = np.arange(VMIN, VMAX + 1e-6, 1.0)
# line_levels = np.arange(VMIN, VMAX + 1e-6, 2.0)
# line_levels = np.arange(VMIN, VMAX + 1e-6, 3.0)
...
# 중간값 필터
size_val = 7
smoothed_data = median_filter(anomaly, size=size_val)
smoothed_ma = np.ma.masked_where(np.isnan(anomaly), smoothed_data)
...
# 2026.05.29 수정
# 컨투어 선 라벨
#ax.clabel(cs, levels=line_levels, inline=True,
#          fontsize=10, fmt='%d', inline_spacing=2)
# plt.clabel(cs, inline=True, fontsize=12, fmt='%d', colors='black')
# plt.clabel(cs, inline=True, fontsize=10, fmt='%d', colors='black')
# labels = plt.clabel(cs, inline=True, fontsize=10, fmt='%d', colors='black')
...
# labels = plt.clabel(cs, inline=True, fontsize=10, fmt='%d', colors='black')
labels = plt.clabel(cs, inline=True, fontsize=10, fmt='%d', colors='black', manual=manual_locs)
for label in labels:
...
# 2026.05.29 수정
# save_path = os.path.join(PNG_DIR, f'gk2a_sst_anomaly_{COMP_MONTH}_{BASE_MONTH}_ko.png')
save_path = PNG_DIR
