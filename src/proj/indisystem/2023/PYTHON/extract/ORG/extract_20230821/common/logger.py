...
def init_formatter():
    ...
    formatter = logging.Formatter('[%(asctime)s | GRID_DECODE_GENERATOR | TP-{} | PRO | %(levelname)s |'
                                  '%(filename)s:%(lineno)s] > %(message)s'.format(hostname))
    ext_formatter = logging.Formatter('[%(asctime)s | GRID_DECODE_GENERATOR | {} | EXT | %(levelname)s |'
                                      '%(filename)s:%(lineno)s] > %(message)s'.format(hostname))
    int_formatter = logging.Formatter('[%(asctime)s | GRID_DECODE_GENERATOR | {} | INT | %(levelname)s | '
                                      '%(filename)s:%(lineno)s] > %(message)s'.format(hostname))
...
def register_logger(level, path=None):
    ...
    :return: 로거
    """
    logger = logging.getLogger("GRID_DECODE_GENERATOR")
