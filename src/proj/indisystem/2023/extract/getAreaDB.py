...
# NREML 서버에서 도메인2 (d02) 해상도, 반면에 SOLAR 서버에서 도메인4 (d04) 해상도 지님
# modelType = KIER-LDAPS-{10M|30M|60M}의 경우 KIER-LDAPS 위경도 정보 사용
# modelType = KIER-RDAPS-{10M|30M|60M}의 경우 KIER-RDAPS 위경도 정보 사용
if re.search('KIER-LDAPS|KIER-RDAPS', modelType, re.IGNORECASE) and (
        pd.to_datetime(srtDate, format='%Y-%m-%d %H:%M') < pd.to_datetime('2012', format='%Y')):
