...
def makeFlamlModel(subOpt=None, xCol=None, yCol=None, trainData=None, testData=None):
    ...
    try:
        ...
        if (subOpt['isOverWrite']) or (len(saveModelList) < 1):
            ...
            # 각 모형에 따른 자동 머신러닝
            # flModel.fit(X_train=trainData[xCol], y_train=trainData[yCol], n_jobs=16)
    ...
    def exec(self):
        ...
        try:
            ...
            for i, CNSMR_NO in enumerate(CNSMR_NO_list):
                ...
                # trainData['is_weekday'] = trainData['is_weekday'].astype(float)
                # trainData['is_spring'] = trainData['is_spring'].astype(float)
                # trainData['is_summer'] = trainData['is_summer'].astype(float)
                # trainData['is_fall'] = trainData['is_fall'].astype(float)
                # trainData['is_winter'] = trainData['is_winter'].astype(float)
                ...
                # plt.plot(prdData['MESURE_DATE_TM'], prdData['prd'], label='예측 (크몽) : {:.3f}'.format(round(np.sqrt(mean_squared_error(prdData['SG_PWRER_USE_AM'], prdData['prd'])), 2)))
                # plt.plot(prdData['MESURE_DATE_TM'], prdData['flaml'], label='예측 (flaml) : {:.3f}'.format(round(np.sqrt(mean_squared_error(prdData['SG_PWRER_USE_AM'], prdData['flaml'])), 2)))
                ...
                mainTitle = '[short-term] [{0:s}-{1:s}]'.format(
                    prdData['SRV'].iloc[0]
                    , CNSMR_NO
                )
                plt.title(mainTitle)
