...
class DtaProcess(object):
    ...
    def exec(self):
        ...
        try:
            ...
            pw_data['MESURE_DATE_TM'] = pd.to_datetime(pw_data['MESURE_DATE_TM'])
            # pw_data.columns

            # inpFile = '{}/{}/{}.csv'.format(globalVar['inpPath'], serviceName, 'FINAL_FORE_DATA')
            inpFile = '{}/{}/{}.csv'.format(globalVar['inpPath'], serviceName, 'FINAL_FORE_DATA_20221110')
            fileList = sorted(glob.glob(inpFile))
            # fore_data = pd.read_csv('FINAL_FORE_DATA.csv')
            fore_data = pd.read_csv(fileList[0])

            fore_data['ANA_DATE'] = pd.to_datetime(fore_data['ANA_DATE'])
            fore_data['DATE_TIME_KST'] = pd.to_datetime(fore_data['DATE_TIME_KST'])
            fore_data['DATE_TIME'] = fore_data['DATE_TIME_KST'] - dtKst

            fore_data['hour'] = fore_data['ANA_DATE'].dt.strftime("%H")
            fore_data = fore_data.loc[
                (fore_data['hour'] == '00')
                ].reset_index(drop=True)


            # fore_data['hourDiff'] = (fore_data['DATE_TIME'] - fore_data['ANA_DATE']) / pd.Timedelta(hours=1)
            # fore_data = fore_data.loc[
            #     (fore_data['hourDiff'] <= 5)
            #     ].reset_index(drop=True)
            ...
            # raw_data = fore_data.merge(pw_data, on=['SRV', 'DATE_TIME_KST'])
            raw_data = fore_data.merge(pw_data, how='left', left_on=['SRV', 'DATE_TIME_KST'], right_on=['SRV', 'MESURE_DATE_TM'])
