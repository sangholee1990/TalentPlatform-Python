    ...
    def exec(self):
        ...
        try:
            ...
            sysOpt = {
                ...
                'reqUrl': 'https://kosis.kr/openapi/Param/statisticsParameterData.do?method=getList&apiKey={apiKey}&itmId=13103871087T1+&objL1=ALL&objL2=ALL&objL3=&objL4=&objL5=&objL6=&objL7=&objL8=&format=json&jsonVD=Y&prdSe=M&startPrdDe={srtDate}&endPrdDe={endDate}&outputFields=ORG_ID+TBL_ID+TBL_NM+OBJ_ID+OBJ_NM+OBJ_NM_ENG+NM+NM_ENG+ITM_ID+ITM_NM+ITM_NM_ENG+UNIT_NM+UNIT_NM_ENG+PRD_SE+PRD_DE+LST_CHN_DE+&orgId=116&tblId=DT_MLTM_2082',
                'reqFile': '/DATA/INPUT/VERSE2026/미분양/전국시군구미분양_%Y%m%d.xlsx',
                'inpFile': '/DATA/INPUT/VERSE2026/미분양/전국시군구미분양_*.xlsx',
                'saveFile': '/DATA/OUTPUT/VERSE2026/TB_UN-SOLD-TREND.csv',
            }
            ...
            colCodeList = ['C1_NM', 'C2_NM', 'DT', 'DT_NM']
            colNameList = ['구분', '시군구', '미분양현황', '시점']
            ...
            for dtDateInfo in dtDateList:
                ...
                try:
                    reqUrl = sysOpt['reqUrl'].format(apiKey=sysOpt['apiKey'], srtDate=dtDateInfo.strftime('%Y%m'), endDate=dtDateInfo.strftime('%Y%m'))
                    ...
                    resData['DT_NM'] = dtDateInfo.strftime('%Y.%m 월')
            ...
            # 코드 정보 읽기
            # *********************************************************************************
            # colNameList = ['시군구', '연도', '입주예정월', '주택명', '세대수', 'sgg']
            # colCodeList = ['sigungu', 'year', 'expComMonth', 'houName', 'numHouse', 'sggCode']
            colNameList = ['연도', '미분양현황', 'sgg']
            colCodeList = ['year', 'unSoldCnt', 'sgg']
            ...
            # 파일 읽기
            # *********************************************************************************
            # inpFile = '{}/{}/{}'.format(globalVar['inpPath'], serviceName, '미분양 추이_전체_*.csv')
            # inpFile = '{}/{}/{}'.format(globalVar['inpPath'], serviceName, '미분양/전국시군구미분양_*.xlsx')
            ...
            for fileInfo in fileList[:1]:
                ...
                if re.search('이전자료', fileNameNoExt, re.IGNORECASE):
                    skiprows = 0
                else:
                    # skiprows = 1
                    skiprows = 0

                data = pd.read_excel(fileInfo, skiprows=skiprows, engine='openpyxl')
                # data = pd.read_csv(fileInfo, encoding='EUC-KR')
                data = data[(data['시군구'] != '계') & (data['구분'] != '전국')].reset_index()

                dtDate = pd.to_datetime(data['시점'], format='%Y.%m 월')
                data['연도'] = dtDate.dt.strftime('%Y')
                data['시도'] = data['구분'].map(sysOpt['metaData'])
                data['sgg'] = data['시도'] + " " + data['시군구']

                dataL1 = data[colNameList].rename(columns=renameDict, inplace=False).dropna(subset=['sgg'])
            ...
            # CSV 통합파일
            # =================================================================
            # saveFile = '{}/{}/{}_{}.csv'.format(globalVar['outPath'], serviceName, datetime.now().strftime("%Y%m%d"), 'TB_UN-SOLD-TREND')
            # saveFile = '{}/{}/{}.csv'.format(globalVar['outPath'], serviceName, 'TB_UN-SOLD-TREND')
            ...
            tableId = f"{credentials.project_id}.DMS01.TB_UN-SOLD-TREND"
