    ...
    def exec(self):
        ...
        try:
            ...
            sysOpt = {
                ...
                'reqUrl': 'https://kosis.kr/openapi/Param/statisticsParameterData.do?method=getList&apiKey={apiKey}&itmId=T1+&objL1=ALL&objL2=&objL3=&objL4=&objL5=&objL6=&objL7=&objL8=&format=json&jsonVD=Y&prdSe=M&startPrdDe={srtDate}&endPrdDe={endDate}&outputFields=ORG_ID+TBL_ID+TBL_NM+OBJ_ID+OBJ_NM+OBJ_NM_ENG+NM+NM_ENG+ITM_ID+ITM_NM+ITM_NM_ENG+UNIT_NM+UNIT_NM_ENG+PRD_SE+PRD_DE+LST_CHN_DE+&orgId=101&tblId=DT_1B040B3',
                'reqFile': '/DATA/INPUT/VERSE2026/월인구/행정구역_시군구_월별_인구수_%Y%m%d.xlsx',
                'inpFile': '/DATA/INPUT/VERSE2026/월인구/행정구역_시군구_월별_인구수_*.xlsx',
                'saveFile': '/DATA/OUTPUT/VERSE2026/TB_MONTH-SGG-POP-TREND.csv',
            }
            ...
            for dtDateInfo in dtDateList:
                ...
                try:
                    ...
                    resData['DT_NM'] = dtDateInfo.strftime('%Y.%m')
            ...
            # 코드 정보 읽기
            # *********************************************************************************
            colNameList = ['행정구역(시군구)별(1)', '행정구역(시군구)별(2)', 'sgg', 'date', '총인구수 (명)']
            colCodeList = ['sigungu', 'dong', 'sgg', 'date', 'cnt']
            ...
            # 파일 읽기
            # *********************************************************************************
            # inpFile = '{}/{}/{}'.format(globalVar['inpPath'], serviceName, '월별 인구변화 추이_전체_*.csv')
            # inpFile = '{}/{}/{}'.format(globalVar['inpPath'], serviceName, '월인구/행정구역_시군구_월별_인구수_*.xlsx')
            ...
            for fileInfo in fileList[:1]:
                ...
                data = pd.read_excel(fileInfo, skiprows=0, engine='openpyxl')
                ...
                data['date'] = pd.to_datetime(data['시점'].astype(str), format='%Y.%m')
            ...
            # CSV 통합파일
            # =================================================================
            # saveFile = '{}/{}/{}_{}.csv'.format(globalVar['outPath'], serviceName, datetime.now().strftime("%Y%m%d"),'TB_MONTH-POP-TREND')
            # saveFile = '{}/{}/{}.csv'.format(globalVar['outPath'], serviceName, 'TB_MONTH-SGG-POP-TREND')
            ...
            tableId = f"{credentials.project_id}.DMS01.TB_MONTH-SGG-POP-TREND"
