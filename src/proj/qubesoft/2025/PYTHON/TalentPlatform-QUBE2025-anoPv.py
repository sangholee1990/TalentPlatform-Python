...
# 프로그램 종료
# ps -ef | grep python | grep TalentPlatform-QUBE2025-anoPv.py | awk '{print $2}' | xargs kill -9
# pkill -f TalentPlatform-QUBE2025-anoPv.py

# 프로그램 시작
# conda activate py38
# cd /SYSTEMS/PROG/PYTHON
# /SYSTEMS/LIB/anaconda3/envs/py39/bin/python TalentPlatform-QUBE2025-anoPv.py
# nohup /SYSTEMS/LIB/anaconda3/envs/py39/bin/python TalentPlatform-QUBE2025-anoPv.py &
        ...
        try:
            ...
            for i, posInfo in posDataL1.iterrows():
                with cfgDb['sessionMake']() as session:
                    ...
                    dataL1['date_time'] = pd.to_datetime(dataL1['date_time'])
                    ...
                    #
                    # # 1. 날짜만 따로 추출하는 컬럼 생성
                    # df_times['date_only'] = df_times['date_time_kst'].dt.date
                    #
                    # # 2. 이상치(Anomaly == 1)가 발생한 날짜들만 추출
                    # anomaly_dates = df_times[df_times['Anomaly'] == 1]['date_only'].unique()
                    #
                    # if len(anomaly_dates) > 0:
                    #     # 3. 첫 번째로 이상치가 발생한 날짜 선택 (변경 가능)
                    #     target_date = anomaly_dates[20]
                    #     print(f"💡 [{target_date}] 기준 1일치 발전량 흐름과 이상치를 시각화합니다 (Matplotlib).")
                    #
                    #     # 4. 해당 날짜(하루)의 데이터만 필터링
                    #     daily_data = df_times[df_times['date_only'] == target_date]
                    #     daily_outliers = daily_data[daily_data['Anomaly'] == 1]
                    #
                    #     # 5. 그래프 사이즈 설정 및 그리기 시작
                    #     plt.figure(figsize=(12, 6))
                    #
                    #     # 전체 데이터 (파란선 + 작은 점)
                    #     plt.plot(daily_data['date_time_kst'], daily_data['pv'],
                    #              marker='o', markersize=4, linestyle='-', color='royalblue', label='Normal PV')
                    #
                    #     # 이상치 데이터 (빨간색 큰 X)
                    #     if not daily_outliers.empty:
                    #         plt.scatter(daily_outliers['date_time_kst'], daily_outliers['pv'],
                    #                     color='red', marker='X', s=150, zorder=5, label='Anomaly (Outlier)')
                    #
                    #     # 6. x축 시간 포맷팅 (시간:분 형태로 깔끔하게 표시)
                    #     plt.gca().xaxis.set_major_formatter(mdates.DateFormatter('%H:%M'))
                    #     plt.gca().xaxis.set_major_locator(mdates.HourLocator(interval=2))  # 2시간 간격 표시
                    #
                    #     # 7. 그래프 꾸미기
                    #     plt.title(f'Daily PV Anomaly Profile - {target_date}', fontsize=16, fontweight='bold')
                    #     plt.xlabel('Time (KST)', fontsize=12)
                    #     plt.ylabel('PV Generation', fontsize=12)
                    #     plt.grid(True, linestyle='--', alpha=0.6)
                    #     plt.legend(fontsize=12)
                    #     plt.xticks(rotation=45)
                    #     plt.tight_layout()
                    #
                    #     # 8. 화면에 출력 (저장하려면 plt.show() 대신 plt.savefig('파일명.png') 사용)
                    #     plt.show()
                    #
                    # else:
                    #     print("탐지된 이상치가 없습니다.")
