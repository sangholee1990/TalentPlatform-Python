...
# 프로그램 종료
# ps -ef | grep python | grep TalentPlatform-QUBE2025-realKimKo.py | awk '{print $2}' | xargs kill -9
# pkill -f TalentPlatform-QUBE2025-realKimKo.py

# 프로그램 시작
# conda activate py39

# cd /SYSTEMS/PROG/PYTHON
# /SYSTEMS/LIB/anaconda3/envs/py39/bin/python /SYSTEMS/PROG/PYTHON/TalentPlatform-QUBE2025-realKimKo.py --cpuCoreNum '5' --srtDate "2026-03-15" --endDate "2026-04-02"
# nohup /SYSTEMS/LIB/anaconda3/envs/py39/bin/python /SYSTEMS/PROG/PYTHON/TalentPlatform-QUBE2025-realKimKo.py --cpuCoreNum '5' --srtDate "2026-03-15" --endDate "2026-04-02" &

# 30 * * * * cd /SYSTEMS/PROG/PYTHON && /SYSTEMS/LIB/anaconda3/envs/py39/bin/python /SYSTEMS/PROG/PYTHON/TalentPlatform-QUBE2025-realKimKo.py --srtDate "$(date -d "2 days ago" +\%Y-\%m-\%d)" --endDate "$(date -d "2 days" +\%Y-\%m-\%d)"
...
def propkimKo(sysOpt, dtDateInfo):
    try:
        # procInfo = mp.current_process()
        umDataList = []
        efList = sysOpt['KIM-KO'][f"ef{dtDateInfo.strftime('%H')}"]
        for ef in efList:
            inpFile = dtDateInfo.strftime(sysOpt['KIM-KO']['inpUmFile']).format(ef=ef)
            fileList = sorted(glob.glob(inpFile))

            for fileInfo in fileList:
                try:
                    grb = xr.open_dataset(fileInfo)
                    grbInfo = grb.isel(Time=0)

                    validIdx = int(ef)
                    analDate = grb.attrs['START_DATE']
                    dtAnalDate = pd.to_datetime(analDate, format='%Y-%m-%d_%H:%M:%S')

                    validDate = grb.isel(Time=0)['XTIME'].values
                    dtValidDate = pd.to_datetime(validDate)

                    row2D = sysOpt['row2D']
                    col2D = sysOpt['col2D']
                    uVec = grbInfo['U10'].values[row2D, col2D]
                    vVec = grbInfo['V10'].values[row2D, col2D]
                    ...
                    pa = grbInfo['PSFC'].values[row2D, col2D] / 100.0
                    ta = grbInfo['T2'].values[row2D, col2D] - 273.15
                    hm = grbInfo['RH2'].values[row2D, col2D]
                    td = mpcalc.dewpoint_from_relative_humidity(ta * units.degC, hm * units.percent).magnitude
                    dirSwr = grbInfo['SWDDIR2'].values[row2D, col2D]
                    difSwr = grbInfo['SWDDIF2'].values[row2D, col2D]
                    swr = np.sum([dirSwr, difSwr], axis=0)
                    skint = grbInfo['TSKIN'].values[row2D, col2D] - 273.15
                    snol = grbInfo['SNOW'].values[row2D, col2D]
                    vis = grbInfo['VIS'].values[row2D, col2D] / 1000.0
                    rain = grbInfo['RAIN'].values[row2D, col2D]
                    fog = grbInfo['FOGFRAC'].values[row2D, col2D]
                    lcld = grbInfo['LCLD'].values[row2D, col2D]
                    mcld = grbInfo['MCLD'].values[row2D, col2D]
                    hcld = grbInfo['HCLD'].values[row2D, col2D]
                    ...
                    umData = xr.Dataset(
                        {
                            ...
                            'rain': (('anaTime', 'time', 'lat', 'lon'), (rain).reshape(1, 1, len(lat1D), len(lon1D))),
                            'fog': (('anaTime', 'time', 'lat', 'lon'), (fog).reshape(1, 1, len(lat1D), len(lon1D))),
                            'lcld': (('anaTime', 'time', 'lat', 'lon'), (lcld).reshape(1, 1, len(lat1D), len(lon1D))),
                            'mcld': (('anaTime', 'time', 'lat', 'lon'), (mcld).reshape(1, 1, len(lat1D), len(lon1D))),
                            'hcld': (('anaTime', 'time', 'lat', 'lon'), (hcld).reshape(1, 1, len(lat1D), len(lon1D))),
                        }
                        , coords={
        ...
        for kk, posInfo in sysOpt['posDataL1'].iterrows():
            ...
            try:
                ...
                dbColList = [
                    ...
                    "hm", "pa", "ta", "td", "wd", "ws", "skint", "snol", "vis", "rain", "fog", "lcld", "mcld", "hcld",
                    "sza", "aza", "et", "turb",
...
def initWorker(cfgFile, cfgDbKey):
    global cfgDb
    import configparser

    config = configparser.ConfigParser(interpolation=None)
    config.read(cfgFile, encoding='utf-8')
    cfgDb = initCfgInfo(config, cfgDbKey)


def subPropProc(sysOpt, cfgDb):
    try:
        cfgUmFile = sysOpt['KIM-KO']['cfgUmFile']
        log.info(f"cfgUmFile : {cfgUmFile}")

        # cfgInfo = pygrib.open(cfgUmFile).select(name='2 metre temperature')[0]
        # lat2D, lon2D = cfgInfo.latlons()

        cfgInfo = xr.open_dataset(cfgUmFile)
        lat2D = cfgInfo.isel(Time=0)['XLAT'].values
        lon2D = cfgInfo.isel(Time=0)['XLONG'].values
        # cfgInfo.isel(Time=0)['T2'].plot()
        # plt.show()
        ...
        pool = Pool(
            ...
            initargs=(sysOpt['cfgFile'], sysOpt['cfgDbKey'])
        )
        ...
        dtSrtDate = pd.to_datetime(sysOpt['srtDate'], format='%Y-%m-%d')
        dtEndDate = pd.to_datetime(sysOpt['endDate'], format='%Y-%m-%d')
        dtDateList = pd.date_range(start=dtSrtDate, end=dtEndDate, freq=sysOpt['KIM-KO']['invDate'])
        for dtDateInfo in reversed(dtDateList):
            # if (dtDateInfo.strftime('%Y-%m-%d %H:%M'), ) in cfgDataList: continue
            pool.apply_async(propkimKo, args=(sysOpt, dtDateInfo))
        pool.close()
        pool.join()
    except Exception as e:
        log.error(f'Exception : {e}')
        ...
        try:
            with cfgDb['sessionMake']() as session:
                with session.begin():
                    ...
                    xColOrg = ['hm', 'pa', 'ta', 'td', 'wd', 'ws', 'skint', 'snol', 'vis', 'rain', 'sza', 'aza', 'et', 'swr']
                    yColOrg = 'pv'
                    ...
                    xCol = ['hm', 'pa', 'ta', 'td', 'wd', 'ws', 'skint', 'snol', 'vis', 'rain', 'fog', 'lcld', 'mcld', 'hcld', 'sza', 'aza', 'et', 'turb', 'ghi_clr', 'dni_clr', 'dhi_clr', 'swr', 'ext_rad']
                    yCol = 'pv'
...
class DtaProcess(object):
    ...
    def exec(self):
        ...
        try:
            ...
            sysOpt = {
                # 시작/종료 시간
                'srtDate': globalVar.get('srtDate', '2026-06-29'),
                'endDate': globalVar.get('endDate', '2026-07-01'),
                'invDate': '1d',
                ...
                # 'cfgFile': 'C:/SYSTEMS/PROG/PYTHON/TalentPlatform-Python/resources/config/system.cfg',
                ...
                'KIM-KO': {
                    ...
                    'ef00': ['000', '001', '002', '003', '004', '005', '015', '016', '017', '018', '019', '020', '021',
                             ...
                             '035', '036', '037', '038'],
                    'ef06': ['000', '001', '002', '003', '004', '005', '009', '010', '011', '012', '013', '014', '015',
                             '016', '017', '018', '019', '020', '021', '022', '023', '024', '025', '026', '027', '028',
                             '029', '030', '031', '032'],
                    'ef12': ['000', '001', '002', '003', '004', '005', '027', '028', '029', '030', '031', '032', '033',
                             '034', '035', '036', '037', '038', '039', '040', '041', '042', '043', '044', '045', '046',
                             '047'],
                    'ef18': ['000', '001', '002', '003', '004', '005', '021', '022', '023', '024', '025', '026', '027',
                ...
                'MODEL': {
                    'orgPycaret': {
                        'saveModelList': "/DATA/AI/*/*/LSH0255-{srv}-final-pycaret-kimKo-*.model.pkl",
                        'saveModel': "/DATA/AI/%Y%m/%d/LSH0255-{srv}-final-pycaret-kimKo-%Y%m%d.model",
                        # 'isOverWrite': True,
                        'isOverWrite': False,
                        'srv': None,
                    ...
                    'orgH2o': {
                        'saveModelList': "/DATA/AI/*/*/LSH0255-{srv}-final-h2o-kimKo-*.model",
                        'saveModel': "/DATA/AI/%Y%m/%d/LSH0255-{srv}-final-h2o-kimKo-%Y%m%d.model",
                        'isInit': False,
                        # 'isOverWrite': True,
                        'isOverWrite': False,
                        'srv': None,
                    ...
                    'lgb': {
                        'saveModelList': "/DATA/AI/*/*/QUBE2025-{srv}-final-lgb-kimKo-*.model",
                        'saveModel': "/DATA/AI/%Y%m/%d/QUBE2025-{srv}-final-lgb-kimKo-%Y%m%d.model",
                        # 'saveImg': "/DATA/AI/%Y%m/%d/QUBE2025-{srv}-final-lgb-kimKo-%Y%m%d.png",
                        # 'isOverWrite': True,
                        'isOverWrite': False,
                        'srv': None,
                        'preDt': datetime.datetime.now(),
                    ...
                    'flaml': {
                        'saveModelList': "/DATA/AI/*/*/QUBE2025-{srv}-final-flaml-kimKo-*.model",
                        'saveModel': "/DATA/AI/%Y%m/%d/QUBE2025-{srv}-final-flaml-kimKo-%Y%m%d.model",
                        # 'saveImg': "/DATA/AI/%Y%m/%d/QUBE2025-{srv}-final-flaml-kimKo-%Y%m%d.png",
                        # 'isOverWrite': True,
                        'isOverWrite': False,
                        'srv': None,
                        'preDt': datetime.datetime.now(),
                    ...
                    'pycaret': {
                        'saveModelList': "/DATA/AI/*/*/QUBE2025-{srv}-final-pycaret-kimKo-*.model.pkl",
                        'saveModel': "/DATA/AI/%Y%m/%d/QUBE2025-{srv}-final-pycaret-kimKo-%Y%m%d.model",
                        # 'saveImg': "/DATA/AI/%Y%m/%d/QUBE2025-{srv}-final-pycaret-kimKo-%Y%m%d.png",
                        # 'isOverWrite': True,
                        'isOverWrite': False,
                        'srv': None,
                        'preDt': datetime.datetime.now(),
