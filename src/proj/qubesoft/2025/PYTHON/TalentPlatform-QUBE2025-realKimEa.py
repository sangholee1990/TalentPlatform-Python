...
def propKimEa(sysOpt, dtDateInfo):
    try:
        # procInfo = mp.current_process()
        umDataList = []
        efList = sysOpt['KIM-EA'][f"ef{dtDateInfo.strftime('%H')}"]
        for ef in efList:
            inpFile = dtDateInfo.strftime(sysOpt['KIM-EA']['inpUmFile']).format(ef=ef)
...
def subPropProc(sysOpt, cfgDb):
    try:
        cfgUmFile = sysOpt['KIM-EA']['cfgUmFile']
        ...
        dtSrtDate = pd.to_datetime(sysOpt['srtDate'], format='%Y-%m-%d')
        dtEndDate = pd.to_datetime(sysOpt['endDate'], format='%Y-%m-%d')
        dtDateList = pd.date_range(start=dtSrtDate, end=dtEndDate, freq=sysOpt['KIM-EA']['invDate'])
        for dtDateInfo in reversed(dtDateList):
            # if (dtDateInfo.strftime('%Y-%m-%d %H:%M'), ) in cfgDataList: continue
            pool.apply_async(propKimEa, args=(sysOpt, dtDateInfo))
        pool.close()
        pool.join()
    except Exception as e:
        log.error(f'Exception : {e}')
    ...
    def exec(self):
        ...
        try:
            ...
            sysOpt = {
                ...
                'MODEL': {
                    'orgPycaret': {
                        'saveModelList': "/DATA/AI/*/*/LSH0255-{srv}-final-pycaret-kimEa-*.model.pkl",
                        'saveModel': "/DATA/AI/%Y%m/%d/LSH0255-{srv}-final-pycaret-kimEa-%Y%m%d.model",
                        # 'isOverWrite': True,
                        'isOverWrite': False,
                        'srv': None,
                    ...
                    'orgH2o': {
                        'saveModelList': "/DATA/AI/*/*/LSH0255-{srv}-final-h2o-kimEa-*.model",
                        'saveModel': "/DATA/AI/%Y%m/%d/LSH0255-{srv}-final-h2o-kimEa-%Y%m%d.model",
                        'isInit': False,
                        # 'isOverWrite': True,
                        'isOverWrite': False,
                        'srv': None,
                    ...
                    'lgb': {
                        'saveModelList': "/DATA/AI/*/*/QUBE2025-{srv}-final-lgb-kimEa-*.model",
                        'saveModel': "/DATA/AI/%Y%m/%d/QUBE2025-{srv}-final-lgb-kimEa-%Y%m%d.model",
                        # 'saveImg': "/DATA/AI/%Y%m/%d/QUBE2025-{srv}-final-lgb-kimEa-%Y%m%d.png",
                        # 'isOverWrite': True,
                        'isOverWrite': False,
                        'srv': None,
                        'preDt': datetime.datetime.now(),
                    ...
                    'flaml': {
                        'saveModelList': "/DATA/AI/*/*/QUBE2025-{srv}-final-flaml-kimEa-*.model",
                        'saveModel': "/DATA/AI/%Y%m/%d/QUBE2025-{srv}-final-flaml-kimEa-%Y%m%d.model",
                        # 'saveImg': "/DATA/AI/%Y%m/%d/QUBE2025-{srv}-final-flaml-kimEa-%Y%m%d.png",
                        # 'isOverWrite': True,
                        'isOverWrite': False,
                        'srv': None,
                        'preDt': datetime.datetime.now(),
                    ...
                    'pycaret': {
                        'saveModelList': "/DATA/AI/*/*/QUBE2025-{srv}-final-pycaret-kimEa-*.model.pkl",
                        'saveModel': "/DATA/AI/%Y%m/%d/QUBE2025-{srv}-final-pycaret-kimEa-%Y%m%d.model",
                        # 'saveImg': "/DATA/AI/%Y%m/%d/QUBE2025-{srv}-final-pycaret-kimEa-%Y%m%d.png",
                        # 'isOverWrite': True,
                        'isOverWrite': False,
                        'srv': None,
                        'preDt': datetime.datetime.now(),
