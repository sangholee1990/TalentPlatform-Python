...
class DtaProcess(object):
    ...
    def exec(self):
        ...
        try:
            ...
            posDataL1 = posData[['id', 'lat', 'lon', 'volume']]
            ...
            for k, modelDirKey in enumerate(modelDirKeyList):
                ...
                for i, posInfo in posDataL1.iterrows():
                    ...
                    #     )
                        ...
                        # 기존 0.759
                        plot_model(modelList[0], plot = 'error')
                        plot_model(modelList[0], plot = 'feature')
                        evaluate_model(modelList[0])
                        predict_model(modelList[0])

                        plot_model(blendModel, plot='error')

                        plot_model(fnlModel, plot='error')
                        # plot_model(fnlModel, plot='feature')
                        evaluate_model(fnlModel)
