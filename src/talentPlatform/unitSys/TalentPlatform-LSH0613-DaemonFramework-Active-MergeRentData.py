...
# /SYSTEMS/LIB/anaconda3/envs/py38/bin/python /SYSTEMS/PROG/PYTHON/IDE/src/talentPlatform/unitSys/TalentPlatform-LSH0613-DaemonFramework-Active-MergeRentData.py
    ...
    def exec(self):
        ...
        try:
            ...
            sysOpt = {
                ...
                '아파트전월세': {
                    'propFile': '/DATA/OUTPUT/LSH0613/전처리/아파트전월세_*_*.csv',
                    'saveFile': '/DATA/OUTPUT/LSH0613/통합/아파트전월세.csv',
                    'tableId': 'DMS01.TB_RENT',
                    'renameDict': {
                        'aptNm': '아파트',
                        'deposit': '보증금액',
                        'monthlyRent': '월세금액',
            ...
            # *********************************************************************************
            # 매매 전세 정보 통합 리스트
            colNameList = ['apt', '전용면적', '평수', '거래 금액', '거래 금액(억원)', '거래가 분류', '보증금액', '월세금액',
                           '층', '건축날짜', '거래유형', '중개사소재지', '계약날짜', '법정동', 'geo', 'sgg',
                           '건축연도', '계약연도', 'key', 'keyDtl', 'aptDtl']

            colCodeList = ['apt', 'capacity', 'area', 'amount', 'amountConv', 'amountType', 'deposit', 'monthlyRent',
                           'floor', 'conDate', 'transMethod', 'broLoc', 'date', 'dong', 'geo', 'sgg',
                           'conYear', 'year', 'key', 'keyDtl', 'aptDtl']

            renameDict = {colName: colCode for colName, colCode in zip(colNameList, colCodeList)}

            # *********************************************************************************
            # 데이터 처리 루프 (매매/전세 순차 처리)
            # *********************************************************************************
            for dataType in ['아파트전월세']:
                log.info(f'[PROCESS] 데이터 유형 처리 중 : {dataType}')

                # *********************************************************************************
                # 파일 읽기
                # *********************************************************************************
                inpFile = sysOpt[dataType]['propFile']
                ...
                for fileInfo in fileList:
                    ...
                    # 각 데이터 타입에 맞는 renameDict 적용
                    data = data.rename(columns=sysOpt[dataType]['renameDict'])

                    # =================================================================
                    # 공통 전처리 로직
                    # =================================================================
                    # 계약날짜: dtYearMonth(201401) + dealDay(23) 조합
                    data['계약날짜'] = pd.to_datetime(data['dtYearMonth'].astype(str) + data['일'].apply(lambda x: f'{int(x):02d}').astype(str), format='%Y%m%d', errors='coerce')
                    data['건축날짜'] = pd.to_datetime(data['건축년도'].astype(str), format='%Y', errors='coerce')
                    data["평수"] = pd.to_numeric(data["전용면적"], errors='coerce').apply(getFloorArea)
                    data['sgg'] = data['addrInfo'] + ' ' + data['d2'].astype(str)
                    data['계약연도'] = data['계약날짜'].dt.year
                    data['건축연도'] = data['건축날짜'].dt.year
                    ...
                    data['aptDtl'] = data['addrInfo'].astype(str) + ' ' + data['d2'].astype(str) + ' ' + data['도로명'].astype(str) + ' ' + data['아파트'].astype(str)

                    # =================================================================
                    # 매매/전세 개별 필드 처리
                    # =================================================================
                    # 전세/월세 처리 ('23,500' 등 문자열 대응)
                    data['거래 금액'] = 0
                    data['거래 금액(억원)'] = 0
                    data['거래가 분류'] = '전월세'
                    data['보증금액'] = pd.to_numeric(data['보증금액'].astype(str).str.replace(',', ''), errors='coerce')
                    data['월세금액'] = pd.to_numeric(data['월세금액'].astype(str).str.replace(',', ''), errors='coerce')

                    # 통합 컬럼 리스트로 재구성
                    dataL1 = data.reindex(columns=colNameList).rename(columns=renameDict, inplace=False)
                    dataL2 = pd.concat([dataL2, dataL1], axis=0)

                # =================================================================
                # CSV 통합파일 저장
                # =================================================================
                saveFile = sysOpt[dataType]['saveFile']
                ...
                try:
                    ...
                    log.error(f'빅쿼리 연결 실패 : {dataType} / {e}')
                    continue
                ...
                # 각 데이터 타입별 목적지 테이블 설정
                tableId = f"{credentials.project_id}.{sysOpt[dataType]['tableId']}"
                
                with open(saveFile, "rb") as file:
                    job = client.load_table_from_file(file, tableId, job_config=jobCfg)
                job.result()
                log.info(f"[CHECK] {dataType} 업로드 완료 -> tableId : {tableId}")
