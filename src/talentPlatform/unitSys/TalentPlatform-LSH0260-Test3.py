...
def initGlobalVar(env=None, contextPath=None, prjName=None):
    ...
    globalVar = {
        ...
        , 'fontPath': contextPath if env in 'local' else os.path.join(contextPath, 'resources', 'config', 'fonInfo')
    }
...
if __name__ == '__main__':
    ...
    try:
        ...
        dtSrtJul = (pd.to_datetime(sysOpt['srtDate'], format='%Y-%m-%d') - timedelta(
            days=sysOpt['movAvgInfo'] - 1)).dayofyear
        ...
        # inpFilePattern = '{:04d}/{:02d}/*swe*.nc'.format(searchYear, searchMonth)
