...
class DtaProcess(object):
    ...
    def exec(self):
        ...
        try:
            ...
            # 코드 정보 읽기
            # *********************************************************************************
            colNameList = ['시군구', '연도', '월', '주택명', '세대수', 'sgg']
            colCodeList = ['sigungu', 'year', 'month', 'houseName', 'houseCnt', 'sgg']
            ...
            for fileInfo in fileList:
                ...
                if re.search('이전자료', fileNameNoExt, re.IGNORECASE):
                    skiprows = 0
                    orgData = pd.read_excel(fileInfo, skiprows=skiprows, engine='openpyxl')
                    data = orgData[orgData['입주예정월'] != 0].reset_index(drop=True)
                    data['연월'] = data['연도'].astype(str) + data['입주예정월'].astype(str).str.zfill(2)
                else:
                    skiprows = 0
            ...
            tableId = f"{credentials.project_id}.DMS01.TB_UP-COM-SUPPLY"
