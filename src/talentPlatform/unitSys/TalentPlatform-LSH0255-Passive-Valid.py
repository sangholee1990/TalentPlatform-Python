    ...
    def exec(self):
        ...
        try:
            ...
            if (platform.system() == 'Windows'):
                ...
                globalVar['figPath'] = 'E:/DATA/OUTPUT/VALID'
            ...
            for idx, posInfo in posDataL1.iterrows():
                ...
                # actData['sDate'] = pd.to_datetime(actData['DATE_TIME']).dt.strftime('%Y%m%d')
                forData['sDate'] = pd.to_datetime(forData['DATE_TIME_KST']).dt.strftime('%Y%m%d')
                forData['type'] = pd.to_datetime(forData['ANA_DATE']).dt.strftime('%Y%m%d%H')
                forData['typeDtl'] = pd.to_datetime(forData['ANA_DATE']).dt.strftime('%H')
                ...
                for i, dtDayInfo in enumerate(dtDayList):
                    ...
                    for key, val in keyList.items():
                        ...
                        typeDtlList = sorted(set(forDataL1['typeDtl']))
                        # typeDtlInfo = typeDtlList[2]
                        for j, typeDtlInfo in enumerate(typeDtlList):
                            forDataL2 = forDataL1.loc[
                                forDataL1['typeDtl'] == typeDtlInfo
                                ]

                            if (len(forDataL2) < 1): continue

                            plt.plot(actDataL1['DATE_TIME_KST'], actDataL1[key], label='실황', marker='o')

                            typeList = sorted(set(forDataL2['type']))
                            for k, typeInfo in enumerate(typeList):
                                forDataL3 = forDataL2.loc[
                                    forDataL2['type'] == typeInfo
                                    ]

                                plt.plot(forDataL3['DATE_TIME_KST'], forDataL3[key], label='기상예보 {}'.format(typeInfo), marker='o')


                            mainTitle = '{}_[{} {}] 기상 정보를 활용한 시계열 ({})'.format(srvId, sDate, typeDtlInfo, val)
                            # saveImg = '{}/{}/{}/{}.png'.format(globalVar['figPath'], colInfo, srvId, mainTitle)
                            saveImg = '{}/{}.png'.format(globalVar['figPath'], mainTitle)
                            log.info('[CHECK] saveImg : {}'.format(saveImg))
                            os.makedirs(os.path.dirname(saveImg), exist_ok=True)

                            # plt.plot(forDataL1['DATE_TIME_KST'], forDataL1['SZA'], 'o', label='기상예보', marker='o')
                            plt.gca().xaxis.set_major_formatter(mdates.DateFormatter('%H'))
                            plt.gca().xaxis.set_major_locator(mdates.HourLocator(interval=2))
                            plt.gcf().autofmt_xdate()
                            plt.xticks(rotation=45, ha='right')
