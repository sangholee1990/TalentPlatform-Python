    ...
    def exec(self):
        ...
        try:
            ...
            for ii, addrInfo in enumerate(sysOpt['addrList']):
                ...
                prvsMntsrInpFile = '{}/{}/{}/{}/{}.csv'.format(globalVar['outPath'], serviceName, '전처리ORG', addrInfo, '건축 인허가-아파트 전월세_*_*')
                ...
                # prvsMntsrData['name'] = prvsMntsrData['addrDtlInfo']
                ...
                prvsMntsrDataL2['보증금'] = prvsMntsrDataL2['보증금(만원)'].astype(str).str.replace(',', '').astype('float')
                # prvsMntsrDataL2['보증금액'] =  pd.to_numeric(prvsMntsrDataL2['보증금액'].astype(str).str.replace(',', ''), errors='coerce')
                prvsMntsrDataL2['전용면적'] = prvsMntsrDataL2['전용면적(㎡)']
                ...
                realPriceInpFile = '{}/{}/{}/{}/{}.csv'.format(globalVar['outPath'], serviceName, '전처리ORG', addrInfo, '건축 인허가-아파트 실거래_*_*')
                ...
                # realPriceData['name'] = realPriceData['addrDtlInfo']
                # realPriceData['name'] = realPriceData['아파트'] + '(' + realPriceData['지번'] + ')'

                # realPriceData
                # result = realPriceData.loc[realPriceData['name'].str.contains('목동신시가지7')]
                ...
                realPriceDataL2['거래금액'] = realPriceDataL2['거래금액(만원)'].astype(str).str.replace(',', '').astype('float')
                realPriceDataL2['전용면적'] = realPriceDataL2['전용면적(㎡)']

                # statData2 = realPriceDataL2.groupby(['name', 'year', '거래금액'], as_index=False)['거래금액'].mean()
                ...
                # statData2 = realPriceDataL4.groupby(['name', 'conYear', 'capacity', 'lat', 'lon', 'year', 'inhuga'], as_index=False)['realPrice'].mean()
                # result = statData2.loc[statData2['name'].str.contains('목동신시가지7')]
                ...
                # prvsMntsrDataL5 = prvsMntsrDataL4.groupby(['name', 'conYear', 'capacity', 'year'], as_index=False)['realBjprice'].mean()
                # realPriceDataL5 = realPriceDataL4.groupby(['name', 'conYear', 'capacity', 'year'], as_index=False)['realPrice'].mean()
                # prvsMntsrDataL5 = prvsMntsrDataL4.groupby(['name', 'conYear', 'capacity', 'year'], as_index=False)['realBjprice'].mean()
                # realPriceDataL5 = realPriceDataL4.groupby(['name', 'conYear', 'capacity', 'year'], as_index=False)['realPrice'].mean()
                ...
                # result3 = realPriceDataL4.loc[realPriceDataL4['name'].str.contains('목동신시가지7')]
                # result4 = prvsMntsrDataL4.loc[prvsMntsrDataL4['name'].str.contains('목동신시가지7')]
                ...
                #     left=prvsMntsrDataL5
                #     , right=realPriceDataL5
                #     # , left_on=['name', 'conYear', 'capacity', 'year']
                #     # , right_on=['name', 'conYear', 'capacity', 'year']
                #     , left_on=['name', 'conYear', 'capacity', 'lat', 'lon', 'year']
                #     , right_on=['name', 'conYear', 'capacity', 'lat', 'lon', 'year']
                #     , how='outer'
                # )

                # prvsMntsrDataL5.columns
                # prvsMntsrDataL5['add']

                # result = prvsMntsrDataL5.loc[prvsMntsrDataL5['name'].str.contains('목동신시가지7')]
                # result2 = realPriceDataL5.loc[realPriceDataL5['name'].str.contains('목동신시가지7')]

                # result3 = data.loc[data['name'].str.contains('목동신시가지7')]
                # result3 = data.loc[data['name'].str.contains('목동신시가지4')]
                # realBjprice    3440
                # realPrice      5288
                # data.isna().sum()
