...
class DtaProcess(object):
    ...
    # 요구사항
    # ================================================
    # Python을 이용한 MK 부동산 기본 및 상세 정보 수집
    ...
    def exec(self):
        ...
        try:
            ...
            admDataL1 = admData.loc[
                ...
                & (admData['법정동명'].str.match('서울특별시 양천구'))
                ].reset_index()
            ...
            for k, row in data.iterrows():
                ...
                try:
                    ...
                    # 단지 매물
                    dtlDanjiTag = htmlDtl.xpath('//*[@id="SY"]/div/div[2]/div[1]/div[2]/div/div[1]/div[1]/div[2]/h2')
                    dtlDanji = dtlDanjiTag[0].text.strip() if (len(dtlDanjiTag) > 0) else None

                    # 단지 종류
                    dtlTypeTag = htmlDtl.xpath('//*[@id="SY"]/div/div[2]/div[1]/div[2]/div/div[1]/div[3]/h3')
                    dtlType = dtlTypeTag[0].text.strip() if (len(dtlTypeTag) > 0) else None

                    # 공급면적/전용면적
                    dtlAreaTagList = htmlDtl.xpath('//*[@id="SY"]/div/div[2]/div[1]/div[2]/div/div[1]/div[2]/p/span')
                    tmpText = []
                    for kk, dtlAreaTag in enumerate(dtlAreaTagList):
                        tmpText.append(dtlAreaTag.text.strip())
                    tmpTextL2 = ''.join(tmpText)
                    tmpTextL3 = re.findall(r'\d+\.\d+', tmpTextL2)

                    dtlSupArea = pd.to_numeric(tmpTextL3[0]) if (len(tmpTextL3[0]) > 0) else None
                    dtlPriArea = pd.to_numeric(tmpTextL3[1]) if (len(tmpTextL3[1]) > 0) else None

                    # 매매가
                    dtlPriceTagList = htmlDtl.xpath(' //*[@id="SY"]/div/div[2]/div[1]/div[2]/div/div[1]/div[3]/p[2]/span')
                    tmpText = []
                    for kk, dtlPriceTag in enumerate(dtlPriceTagList):
                        tmpText.append(dtlPriceTag.text.strip())
                    tmpTextL2 = ''.join(tmpText).replace(",", "").replace("만원", "")

                    dtlPrice = pd.to_numeric(tmpTextL2) / 10000 if (len(tmpTextL2) > 0) else None
                ...
                data.loc[k, 'dtlDanji'] = dtlDanji
                data.loc[k, 'dtlType'] = dtlType
                data.loc[k, 'dtlSupArea'] = dtlSupArea
                data.loc[k, 'dtlPriArea'] = dtlPriArea
                data.loc[k, 'dtlPrice'] = dtlPrice
                data.loc[k, 'admCode'] = admInfo['법정동코드']
                data.loc[k, 'admAddr'] = admInfo['법정동명']

                data.loc[k, 'urlDtlInfo'] = sysOpt['prefixDtlUrl'] + urlDtlInfo
                print(sysOpt['prefixDtlUrl'] + urlDtlInfo)

                # dataL1 = pd.concat([dataL1, data], axis=0, ignore_index=True)

            saveFile = '{}/{}/{}_{}.xlsx'.format(globalVar['outPath'], serviceName, '서울특별시 시군구에 따른 매물 및 부동산 중개업소 목록', datetime.now().strftime('%Y%m%d%H'))
