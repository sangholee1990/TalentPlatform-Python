...
class DtaProcess(object):
    ...
    # conda activate
    # python3 TalentPlatform-LSH0255-ASOS-L1.py --inpPath "/SYSTEMS/OUTPUT" --outPath "/SYSTEMS/OUTPUT" --srtDate "2020-09"
    ...
    def exec(self):
        ...
        try:
            ...
            for i, dtDateInfo in enumerate(dtDateList):
                ...
                # plt.contourf(ds['lon'], ds['lat'], ds['TA'][0, :, :], levels=100)
                # plt.colorbar()
                # plt.show()

                saveFile = '{}/TEST/OBS/ASOS_OBS_{}.nc'.format(globalVar['outPath'], pd.to_datetime(dtDateInfo).strftime('%Y%m%d%H'))
            ...
            # saveFile = '{}/TEST/OBS/ASOS_OBS_{}.nc'.format(globalVar['outPath'], pd.to_datetime(dtDateInfo).strftime('%Y%m'))
