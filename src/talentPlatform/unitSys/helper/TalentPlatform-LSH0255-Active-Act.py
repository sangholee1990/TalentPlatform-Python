...
def subAtmosActData(cfgInfo, posDataL1, dtIncDateList):
    ...
    try:
        ...
        for ii, dtIncDateInfo in enumerate(dtIncDateList):
            ...
            # 결측값 제거
            dataL3 = dataL2
            dataL3['dtDateKst'] = pd.to_datetime(dataL3['TM'].astype(str), format='%Y%m%d%H%M')
            dataL3['dtDate'] = dataL3['dtDateKst'] - dtKst
...
def setPrdActDataDB(cfgInfo, dbData):
    ...
    try:
        ...
        for k, dbInfo in dbData.iterrows():
            ...
            session.execute(
                """
                UPDATE `{}` SET ML = '{}', DL = '{}', ML2 = '{}', DL2 = '{}', MOD_DATE = '{}'
                WHERE SRV = '{}' AND DATE_TIME = '{}';
                """.format(selDbTable
                           , dbInfo['ML'], dbInfo['DL'], dbInfo['ML2'], dbInfo['DL2'], dbInfo['MOD_DATE']
                           , dbInfo['SRV'], dbInfo['DATE_TIME'])
            )
...
class DtaProcess(object):
    ...
    def exec(self):
        ...
        try:
            ...
            dtPvDateList = pd.date_range(start=dtSrtDate, end=dtEndDate, freq=Hour(1))
            ...
            for idx, posInfo in posDataL1.iterrows():
                ...
                # prdData['ML'] = predict_model(mlModel, data=prdData)['Label']
                ...
                # prdData['DL'] = dlModel.predict(h2o.H2OFrame(prdData[xCol].dropna())).as_data_frame()
                # prdVal = dlModel.predict(h2o.H2OFrame(prdData[xCol].dropna())).as_data_frame()
                # prdData['DL'] = prdVal['predict'].values
                tmpData = prdData[xCol].dropna()
                prdVal = dlModel.predict(h2o.H2OFrame(tmpData)).as_data_frame()
                prdVal.index = tmpData.index
                tmpData['DL'] = prdVal.values

                prdData = pd.merge(left=prdData, right=tmpData['DL'], left_index=True, right_index=True, how='left', suffixes=('_x', ''))

                # ******************************************************************************************************
                # 머신러닝
                # ******************************************************************************************************
                xCol = ['CA_TOT', 'HM', 'PA', 'TA', 'TD', 'WD', 'WS', 'SWR', 'SZA', 'AZA', 'ET', 'GHI_CLR', 'GHI_POA']
                yCol = 'PV'

                sysOpt['mlModel'].update(
                    {
                        'srvId' : srvId
                        , 'modelKey' : 'AI-ACT-20220716'
                        , 'isOverWrite': False
                    }
                )
                ...
                # prdData['DL2'] = dlModel.predict(h2o.H2OFrame(prdData[xCol])).as_data_frame()
                # prdVal = dlModel.predict(h2o.H2OFrame(prdData[xCol])).as_data_frame()
                # prdData['DL2'] = prdVal['predict'].values
                tmpData = prdData[xCol].dropna()
                prdVal = dlModel.predict(h2o.H2OFrame(tmpData)).as_data_frame()
                prdVal.index = tmpData.index
                tmpData['DL2'] = prdVal.values

                prdData = pd.merge(left=prdData, right=tmpData['DL2'], left_index=True, right_index=True, how='left', suffixes=('_x', ''))

                # DB 삽입
                setPrdActDataDB(cfgInfo, prdData)
