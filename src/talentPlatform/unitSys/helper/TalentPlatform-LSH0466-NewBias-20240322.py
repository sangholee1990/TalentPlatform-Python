...
def parallel_bias_adjustments_historical(obsDataL3U, modDataL3CU):
    # 역사적 및 미래 보정 작업을 위한 태스크 정의
    tasks = [
        {'method': 'quantile_delta_mapping', 'obs': obsDataL3U['pr'], 'simh': modDataL3CU['pr'],
         'simp': modDataL3CU['pr'], 'n_quantiles': 1000, 'kind': '*'},
        {'method': 'linear_scaling', 'obs': obsDataL3U['pr'], 'simh': modDataL3CU['pr'], 'simp': modDataL3CU['pr'],
         'group': 'time.month', 'kind': '*'},
        # {'method': 'variance_scaling', 'obs': obsDataL3U['pr'], 'simh': modDataL3CU['pr'], 'simp': modDataL3CU['pr'],
        #  'group': 'time.dayofyear', 'kind': '+'},
        {'method': 'delta_method', 'obs': obsDataL3U['pr'], 'simh': modDataL3CU['pr'], 'simp': modDataL3CU['pr'],
         'group': 'time.month', 'kind': '*'},
        {'method': 'quantile_mapping', 'obs': obsDataL3U['pr'], 'simh': modDataL3CU['pr'], 'simp': modDataL3CU['pr'],
         'n_quantiles': 1000, 'kind': '*'},
...
def adjust_future_periods_dask(client, obs, sim_hist, sim_future, methods, n_quantiles=100, group='time.month',
    ...
    for start_year, end_year in future_periods:
        ...
        # 연도별로 청크 생성
        years = np.unique(simp_period.time.dt.year.values)

        for year in years:
            logging.info(f"Processing year: {year}")
            # 해당 연도의 데이터 선택
            obs_chunk = obs.sel(time=slice(f"{year}-01-01", f"{year}-12-31"))
            sim_hist_chunk = sim_hist.sel(time=slice(f"{year}-01-01", f"{year}-12-31"))
            simp_period_chunk = simp_period.sel(time=slice(f"{year}-01-01", f"{year}-12-31"))

            tasks = []
            for method in methods:
                logging.info(f"Submitting task for method: {method} for year {year}")
    ...
    def exec(self):
        ...
        try:
            ...
            for keyInfo in keyList:
                ...
                # 병렬 처리를 위한 편향 조정 호출
                log.info("Starting parallel bias adjustments for historical data.")
                historical_results = parallel_bias_adjustments_historical(obsDataL3U, modDataL3CU)
                log.info("Parallel bias adjustments completed for historical data.")

                QDM_Hisresult = historical_results['quantile_delta_mapping']
                LS_Hisresult = historical_results['linear_scaling']
                DS_Hisresult = historical_results['delta_method']
                QM_Hisresult = historical_results['quantile_mapping']

                saveFile = '{}/{}/{}_{}_{}.nc'.format(globalVar['outPath'], keyInfo, 'PAST-MBC', 'QDM', keyInfo)
                os.makedirs(os.path.dirname(saveFile), exist_ok=True)
                QDM_Hisresult.to_netcdf(saveFile)
                log.info(f'[CHECK] saveFile : {saveFile}')

                saveFile = '{}/{}/{}_{}_{}.nc'.format(globalVar['outPath'], keyInfo, 'PAST-MBC', 'LS', keyInfo)
                os.makedirs(os.path.dirname(saveFile), exist_ok=True)
                LS_Hisresult.to_netcdf(saveFile)
                log.info(f'[CHECK] saveFile : {saveFile}')

                saveFile = '{}/{}/{}_{}_{}.nc'.format(globalVar['outPath'], keyInfo, 'PAST-MBC', 'DS', keyInfo)
                os.makedirs(os.path.dirname(saveFile), exist_ok=True)
                DS_Hisresult.to_netcdf(saveFile)
                log.info(f'[CHECK] saveFile : {saveFile}')

                saveFile = '{}/{}/{}_{}_{}.nc'.format(globalVar['outPath'], keyInfo, 'PAST-MBC', 'DS', keyInfo)
                os.makedirs(os.path.dirname(saveFile), exist_ok=True)
                QM_Hisresult.to_netcdf(saveFile)
                log.info(f'[CHECK] saveFile : {saveFile}')

                # 결과 데이터 추출 및 병합, 이름 변경
                QDM_Hisresult_Con = xr.merge([historical_results['quantile_delta_mapping'], contDataL4]).rename(
                    {'pr': 'QDM'})
                LS_Hisresult_Con = xr.merge([historical_results['linear_scaling'], contDataL4]).rename({'pr': 'LS'})
                # VS_Hisresult_Con = xr.merge([historical_results['VS_His'], contDataL4]).rename({'pr': 'VS_His'})
                DM_Hisresult_Con = xr.merge([historical_results['delta_method'], contDataL4]).rename({'pr': 'DM'})
                QM_Hisresult_Con = xr.merge([historical_results['quantile_mapping'], contDataL4]).rename({'pr': 'QM'})

                varList = ['OBS', 'ERA', 'bias_pr']
                contIdxList = np.unique(QDM_Hisresult_Con['contIdx'])

                lat1D = QDM_Hisresult_Con['lat'].values
                lon1D = QDM_Hisresult_Con['lon'].values
                time1D = QDM_Hisresult_Con['time'].values

                QDMDataL2 = xr.Dataset(
                    {
                        'OBS': (('time', 'lat', 'lon'),
                                (obsDataL3U['pr'].values).reshape(len(time1D), len(lat1D), len(lon1D)))
                        , 'ERA': (('time', 'lat', 'lon'),
                                  (modDataL3CU['pr'].values).reshape(len(time1D), len(lat1D), len(lon1D)))
                        , 'bias_pr': (('time', 'lat', 'lon'),
                                      (QDM_Hisresult_Con['QDM'].transpose('time', 'lat', 'lon').values).reshape(
                                          len(time1D),
                                          len(lat1D),
                                          len(lon1D)))
                        # , 'isLand': (('time', 'lat', 'lon'), np.tile(contDataL4['isLand'].values[np.newaxis, :, :], (len(time1D), 1, 1)).reshape(len(time1D), len(lat1D), len(lon1D)))
                        , 'contIdx': (('time', 'lat', 'lon'), np.tile(contDataL4['contIdx'].values[np.newaxis, :, :],
                                                                      (len(time1D), 1, 1)).reshape(len(time1D),
                                                                                                   len(lat1D),
                                                                                                   len(lon1D)))
                    }
                    , coords={
                ...
                LSDataL2 = xr.Dataset(
                    {
                        ...
                        , 'ERA': (('time', 'lat', 'lon'),
                                  (modDataL3CU['pr'].values).reshape(len(time1D), len(lat1D), len(lon1D)))
                        , 'bias_pr': (('time', 'lat', 'lon'),
                                      (LS_Hisresult_Con['LS'].transpose('time', 'lat', 'lon').values).reshape(
                                          len(time1D),
                                          len(lat1D),
                                          len(lon1D)))
                        # , 'isLand': (('time', 'lat', 'lon'), np.tile(contDataL4['isLand'].values[np.newaxis, :, :], (len(time1D), 1, 1)).reshape(len(time1D), len(lat1D), len(lon1D)))
                ...
                DMDataL2 = xr.Dataset(
                    {
                        ...
                        , 'ERA': (('time', 'lat', 'lon'),
                                  (modDataL3CU['pr'].values).reshape(len(time1D), len(lat1D), len(lon1D)))
                        , 'bias_pr': (('time', 'lat', 'lon'),
                                      (DM_Hisresult_Con['DM'].transpose('time', 'lat', 'lon').values).reshape(
                                          len(time1D),
                                          len(lat1D),
                                          len(lon1D)))
                        # , 'isLand': (('time', 'lat', 'lon'), np.tile(contDataL4['isLand'].values[np.newaxis, :, :], (len(time1D), 1, 1)).reshape(len(time1D), len(lat1D), len(lon1D)))
                ...
                QMDataL2 = xr.Dataset(
                    {
                        ...
                        , 'ERA': (('time', 'lat', 'lon'),
                                  (modDataL3CU['pr'].values).reshape(len(time1D), len(lat1D), len(lon1D)))
                        , 'bias_pr': (('time', 'lat', 'lon'),
                                      (QM_Hisresult_Con['QM'].transpose('time', 'lat', 'lon').values).reshape(
                                          len(time1D),
                                          len(lat1D),
                                          len(lon1D)))
                        # , 'isLand': (('time', 'lat', 'lon'), np.tile(contDataL4['isLand'].values[np.newaxis, :, :], (len(time1D), 1, 1)).reshape(len(time1D), len(lat1D), len(lon1D)))
                ...
                QDMDataL3 = QDMDataL2.to_dataframe().reset_index(drop=False)
                LSDataL3 = LSDataL2.to_dataframe().reset_index(drop=False)
                # VSDataL3 = VSDataL2.to_dataframe().reset_index(drop=False)
                DMDataL3 = DMDataL2.to_dataframe().reset_index(drop=False)
                QMDataL3 = QMDataL2.to_dataframe().reset_index(drop=False)
                # DQMDataL3 = DQMDataL2.to_dataframe().reset_index(drop=False)
                ...
                for varInfo in varList:
                    ...
                    for contIdx in contIdxList:
                        ...
                        QDMDataL5 = QDMDataL3.loc[QDMDataL3['contIdx'] == contIdx].reset_index(drop=True)
                        if len(QDMDataL5) < 0: continue
                        LSDataL5 = LSDataL3.loc[LSDataL3['contIdx'] == contIdx].reset_index(drop=True)
                        if len(LSDataL5) < 0: continue
                        # VSDataL5 = VSDataL3.loc[VSDataL3['contIdx'] == contIdx].reset_index(drop=True)
                        # if len(VSDataL5) < 0: continue
                        DMDataL5 = DMDataL3.loc[DMDataL3['contIdx'] == contIdx].reset_index(drop=True)
                        if len(DMDataL5) < 0: continue
                        QMDataL5 = QMDataL3.loc[QMDataL3['contIdx'] == contIdx].reset_index(drop=True)
                        if len(QMDataL5) < 0: continue
                        # #DQMDataL5 = DQMDataL3.loc[DQMDataL3['contIdx'] == contIdx].reset_index(drop=True)
                        # if len(DQMDataL5) < 0: continue
                        ...
                        QDMDataL6 = QDMDataL5[selCol].dropna().pivot(index=['time'], columns=['lon', 'lat'])
                        LSDataL6 = LSDataL5[selCol].dropna().pivot(index=['time'], columns=['lon', 'lat'])
                        # VSDataL6 = VSDataL5[selCol].dropna().pivot(index=['time'], columns=['lon', 'lat'])
                        DMDataL6 = DMDataL5[selCol].dropna().pivot(index=['time'], columns=['lon', 'lat'])
                        QMDataL6 = QMDataL5[selCol].dropna().pivot(index=['time'], columns=['lon', 'lat'])
                        # DQMDataL6 = DQMDataL5[selCol].dropna().pivot(index=['time'], columns=['lon', 'lat'])

                        saveCsvFile1 = '{}/{}/{}-{}_{}-{}_{}.csv'.format(globalVar['outPath'], keyInfo, 'PAST-MBC',
                                                                     varInfo, 'QDM', sheetName, keyInfo)
                        os.makedirs(os.path.dirname(saveCsvFile1), exist_ok=True)

                        QDMcsvData = QDMDataL6.reset_index(drop=False)
                        QDMcsvData.to_csv(saveCsvFile1, index=False)
                        log.info(f'[CHECK] saveCsvFile : {saveCsvFile1}')

                        saveCsvFile2 = '{}/{}/{}-{}_{}-{}_{}.csv'.format(globalVar['outPath'], keyInfo, 'PAST-MBC',
                                                                     varInfo, 'LS', sheetName, keyInfo)
                        os.makedirs(os.path.dirname(saveCsvFile2), exist_ok=True)
                        LScsvData = LSDataL6.reset_index(drop=False)
                        LScsvData.to_csv(saveCsvFile2, index=False)
                        log.info(f'[CHECK] saveCsvFile : {saveCsvFile2}')

                        saveCsvFile4 = '{}/{}/{}-{}_{}-{}_{}.csv'.format(globalVar['outPath'], keyInfo, 'PAST-MBC',
                                                                     varInfo, 'DM', sheetName, keyInfo)
                        os.makedirs(os.path.dirname(saveCsvFile4), exist_ok=True)

                        DMcsvData = DMDataL6.reset_index(drop=False)
                        DMcsvData.to_csv(saveCsvFile4, index=False)
                        log.info(f'[CHECK] saveCsvFile : {saveCsvFile4}')

                        saveCsvFile5 = '{}/{}/{}-{}_{}-{}_{}.csv'.format(globalVar['outPath'], keyInfo, 'PAST-MBC',
                                                                     varInfo, 'QM', sheetName, keyInfo)
                        os.makedirs(os.path.dirname(saveCsvFile5), exist_ok=True)

                        QMcsvData = QMDataL6.reset_index(drop=False)
                        QMcsvData.to_csv(saveCsvFile5, index=False)
                        log.info(f'[CHECK] saveCsvFile : {saveCsvFile5}')

                del obsDataL3U, modDataL3CU
                ...
                future_results = adjust_future_periods_dask(obs=obsDataL3U, sim_hist=modDataL3CU, sim_future=simDataL3ProcessedU, methods=methods, client=client)
