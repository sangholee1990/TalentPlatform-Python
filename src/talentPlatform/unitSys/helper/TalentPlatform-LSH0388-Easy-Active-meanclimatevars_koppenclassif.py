...
sysOpt = {
    ...
    , 'dateList' : ['1954', '1964', '1974', '1984', '1994', '2004', '2030', '2040', '2050', '2060', '2070', '2080', '2090']
    , 'keyList' : ['ACCESS-CM2']
...
for i, keyInfo in enumerate(sysOpt['keyList']):
    print('[CHECK] keyInfo : {}'.format(keyInfo))
    ...
    # inpFile = '{}/{}/tas_{}_land.nc'.format(globalVar['inpPath'], serviceName, keyInfo)
    inpFile = '{}/{}/{} historical_*_ts.nc'.format(globalVar['inpPath'], serviceName, keyInfo)
    ...
    # inpFile = '{}/{}/pr_{}_land.nc'.format(globalVar['inpPath'], serviceName, keyInfo)
    inpFile = '{}/{}/{} historical_*_pr.nc'.format(globalVar['inpPath'], serviceName, keyInfo)
    fileList = sorted(glob.glob(inpFile))
    in_netcdf_pr = fileList[0]

    # dateInfo = 2040
    for j, dateInfo in enumerate(sysOpt['dateList']):

        sysOpt['srtDate'] = pd.to_datetime(dateInfo, format='%Y').strftime('%Y-01')
        sysOpt['endDate'] = (pd.to_datetime(dateInfo, format='%Y') + pd.offsets.DateOffset(years=10)).strftime('%Y-12')

        print('[CHECK] srtDate ~ endDate : {} ~ {}'.format(sysOpt['srtDate'], sysOpt['endDate']))
        ...
        if len(data['time'].values) < 1: continue
        ...
        try:
            ...
            for j in range(rows * cols - cols, -1, -cols):  # rows, bottoms up
                for i in range(0, cols):  # cols
                    ...
                    # print(f'[CHECK] row : {row}, colIdx : {colIdx}')
                row += 1
            ...
            convlon1D = sorted((lon1D + 180) % 360 - 180)
            ...
            minYear = pd.to_datetime(sysOpt['srtDate'], format='%Y-%m').strftime('%Y')
            maxYear = pd.to_datetime(sysOpt['endDate'], format='%Y-%m').strftime('%Y')

            # NetCDF 생성
            # saveFile = '{}/{}/{}.nc'.format(globalVar['outPath'], serviceName, 'koppen_MME-land')
            saveFile = '{}/{}/koppen_{}-land_{}-{}.nc'.format(globalVar['outPath'], serviceName, keyInfo, minYear, maxYear)
            os.makedirs(os.path.dirname(saveFile), exist_ok=True)
            dataL2.to_netcdf(saveFile)
            print(f'[CHECK] saveFile : {saveFile}')

            # CSV 생성
            saveFile = '{}/{}/koppen_{}-land_{}-{}.csv'.format(globalVar['outPath'], serviceName, keyInfo, minYear, maxYear)
            os.makedirs(os.path.dirname(saveFile), exist_ok=True)
            ...
            mainTitle = f'koppen_{keyInfo}-land_{minYear}-{maxYear}'
