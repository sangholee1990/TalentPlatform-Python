...
class DtaProcess(object):
    ...
    def exec(self):
        ...
        try:
            ...
            # for i, stnDataInfo in stnDataL1.iterrows():
            #     print(i, stnDataInfo['lon'], stnDataInfo['lat'])
            ...
            for i, dtIncDateInfo in enumerate(dtIncDateList):
                ...
                inpFilePattern = 'TEST/OBS/ASOS_OBS_{}.nc'.format(dtDateYmdH)
                ...
                # for i, posInfo in posDataL1.iterrows():
                    # print(posInfo)
                    # lon1D = np.array(posInfo['lon']).reshape(1)
                    # lat1D = np.array(posInfo['lat']).reshape(1)
                lon1D = np.array(posDataL1['lon']).reshape(len(posDataL1))
                lat1D = np.array(posDataL1['lat']).reshape(len(posDataL1))

                # selNearVal = ds.sel(lat=posInfo['lat'], lon=posInfo['lon'], method='nearest')
                # selIntpVal = ds.interp(lat=posInfo['lat'], lon=posInfo['lon'])
