...
class DtaProcess(object):
    ...
    def exec(self):
        ...
        try:
            ...
            # db_connection_str = 'mysql://dms02user01:devel!@211.227.85.209:3066/DMS02'
            # DB 연결 정보
            ...
            db_connection_str = 'mysql://{0}:{1}@{2}:{3}/{4}'.format(user, pwd, host, port, dbName)
            ...
            for i, dtIncDateInfo in enumerate(dtIncDateList):
                ...
                for j, fileInfo in enumerate(fileList):
                    ...
                    for i, posInfo in posDataL1.iterrows():
                        ...
                        keyChk = db_connection.execute(
                            """
                            SELECT COUNT(*) AS CNT
                            FROM TB_FOR_DATA
                            WHERE  SRV = %s AND ANA_DATE = %s AND DATE_TIME = %s
                            """
                            , (inpDataL1['SRV'][0], inpDataL1['anaTime'][0], inpDataL1['dtDate'][0])
                        ).fetchone()
                        ...
                        saveDlModel = '{}/{}/{}-SRV{:05d}-{}-{}-{}-{}.model'.format(globalVar['modelPath'], modelDirKey, serviceName, posId, 'final', 'h2o', 'for', '20220220')
                        ...
                        res = db_connection.execute(
                            ...
                            WHERE  SRV = %s AND ANA_DATE = %s AND DATE_TIME = %s
                            """
                            , (dbData['SRV'][0], dbData['ANA_DATE'][0], dbData['DATE_TIME'][0])
                        ).fetchone()
                        ...
                        # 삽입
                        dbData.to_sql(name='TB_FOR_DATA', con=db_connection, if_exists='append', index=False)
        except Exception as e:
            log.error("Exception : {}".format(e))
            raise e
