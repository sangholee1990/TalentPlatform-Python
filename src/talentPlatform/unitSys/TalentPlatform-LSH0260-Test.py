...
if __name__ == '__main__':
    ...
    try:
        ...
        for yearInfo in list(range(srtYear, endYear + 1)):
            for monthInfo in list(range(srtMonth, endMonth + 1)):
                inpFilePattern = 'swe/{:04d}/{:02d}/*swe*.nc'.format(yearInfo, monthInfo)
                inpFile = '{}/{}/{}'.format(globalVar['inpPath'], serviceName, inpFilePattern)
                fileList = glob.glob(inpFile)
                clmFileList.extend(fileList)

        clmDs = xr.open_mfdataset(clmFileList)
        clmDsSwe = clmDs['swe']
