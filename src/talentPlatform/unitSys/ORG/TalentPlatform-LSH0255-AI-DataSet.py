...
class DtaProcess(object):
    ...
    def exec(self):
        ...
        try:
            ...
            # 예측 데이터
            dtSrtDate = pd.to_datetime('2019-01-01', format='%Y-%m-%d')
            ...
            #         lat = posLat
            #         lon = posLon
            #         pa = trainDataL2._get_value(i, 'PA')
            ...
            #
            #     saveXlsxFile = '{}/{}-SRV{:03d}-{}-{}-{}.xlsx'.format(globalVar['outPath'], serviceName, posId, 'final', 'proc', 'act')
            #     log.info("[CHECK] saveXlsxFile : {}".format(saveXlsxFile))
            #     trainDataL2.to_excel(saveXlsxFile, index=False)

                # idx = inpData.loc[inpDataL2['dtDate_x'] == pd.to_datetime('2021-01-01', format='%Y-%m-%d')].index.to_numpy()[0]
                # trainData, testData = inpDataL2[0:idx], inpDataL2[idx:len(inpDataL2)]
                # trainDataL3 = trainDataL2[['CA_TOT', 'HM', 'PA', 'TA', 'TD', 'WD', 'WS', 'SWR', 'sza', 'aza', 'et', 'pv']]
                # trainDataL4 = trainDataL3.dropna().reset_index(drop=True)
                ...
                for k, fileInfo in enumerate(fileList):
                    ...
                    for j, dtAnaTimeInfo in enumerate(dtAnaTimeList):
                        ...
                        try:
                            ...
                            umDataL5 = umDataL4[['dtDateKst', 'CA_TOT', 'HM', 'PA', 'TA', 'TD', 'WD', 'WS', 'SWR']]

                            umDataL6 = umDataL5
                            for i in umDataL6.index:
                                lat = posLat
                                lon = posLon
                                pa = umDataL6._get_value(i, 'PA')
                            ...
                            # saveCsvFile = '{}/{}/{}-SRV{:03d}-{}-{}-{}-{}.csv'.format(globalVar['outPath'], 'FOR', serviceName, posId, 'final', 'proc', 'for', pd.to_datetime(dtAnaTimeInfo).strftime('%Y%m%d'))
                            # os.makedirs(os.path.dirname(saveCsvFile), exist_ok=True)
                            # umDataL7.to_csv(saveCsvFile, index=False)

                        except Exception as e:
                            log.error("Exception : {}".format(e))

                    saveXlsxFile = '{}/{}/{}-SRV{:03d}-{}-{}-{}-{}-{}.xlsx'.format(globalVar['outPath'], 'FOR', serviceName, posId, 'final', 'proc', 'for', pd.to_datetime(min(dtAnaTimeList)).strftime('%Y%m%d'), pd.to_datetime(max(dtAnaTimeList)).strftime('%Y%m%d'))
                    log.info("[CHECK] saveXlsxFile : {}".format(saveXlsxFile))
            ...
            for i, posInfo in posDataL1.iterrows():
                ...
                inpFilePattern = '*SRV{:03d}*.xlsx'.format(posId)
                inpFile = '{}/{}/{}'.format(globalVar['outPath'], 'FOR', inpFilePattern)
                fileList = sorted(glob.glob(inpFile))

                umDataL9 = pd.DataFrame()
                for j, fileInfo in enumerate(fileList):
                    tmpData = pd.read_excel(fileInfo)

                    umDataL9 = umDataL9.append(tmpData)

                saveXlsxFile = '{}/{}-SRV{:03d}-{}-{}-{}.xlsx'.format(globalVar['outPath'], serviceName, posId, 'final', 'proc', 'for')
