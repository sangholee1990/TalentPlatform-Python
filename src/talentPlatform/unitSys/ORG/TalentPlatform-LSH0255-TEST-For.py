    ...
    try:
        ...
        for k, dbInfo in dbData.iterrows():
            ...
            if (resChk.loc[0, 'CNT'] > 0):
                ...
                session.execute(
                    """
                    INSERT INTO `{}` (SRV, ANA_DATE, DATE_TIME, DATE_TIME_KST, CA_TOT, HM, PA, TA, TD, WD, WS, SZA, AZA, ET, TURB, GHI_CLR, DNI_CLR, DHI_CLR, GHI_POA, DNI_POA, DHI_POA, SWR, REG_DATE)
                    VALUES ('{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}')
                    """.format(selDbTable
...
def subModelInpData(sysOpt, cfgInfo, srvId, posVol, dtIncDateList):
    ...
    try:
        ...
        # prdData = tmpData.loc[
        #     (tmpData['ANA_DATE'].dt.strftime("%Y-%m-%d") == dtDateInfo.strftime("%Y-%m-%d"))
        # ].drop_duplicates(['SRV', 'ANA_DATE', 'DATE_TIME_KST']).sort_values(by=['SRV', 'ANA_DATE', 'DATE_TIME_KST'], axis=0).reset_index(drop=True)
...
def subVisPrd(subOpt, prdData):
    ...
    try:
        ...
        minData = pd.to_datetime(prdData['DATE_TIME_KST']).min().strftime("%Y%m%d")
        maxData = pd.to_datetime(prdData['DATE_TIME_KST']).max().strftime("%Y%m%d")
...
def setPrdDataDB(cfgInfo, dbData):
    ...
    try:
        ...
        for k, dbInfo in dbData.iterrows():
            ...
            # selDbTable = 'TEST_TB_FOR_DATA_{}'.format(iAnaYear)
            selDbTable = 'QA_TB_FOR_DATA_{}'.format(iAnaYear)
            ...
            if (resChk.loc[0, 'CNT'] > 0):
                ...
                session.execute(
                    """
                    UPDATE `{}`
                    SET DATE_TIME_KST = '{}', CA_TOT = '{}', HM = '{}', PA = '{}', TA = '{}', TD = '{}', WD = '{}', WS = '{}', SZA = '{}', AZA = '{}', ET = '{}'
                    , SWR = '{}', ML2 = '{}', DL2 = '{}', MOD_DATE = '{}'
                    WHERE SRV = '{}' AND ANA_DATE = '{}' AND DATE_TIME = '{}';
                    """.format(selDbTable
                               , dbInfo['DATE_TIME_KST'], dbInfo['CA_TOT'], dbInfo['HM'], dbInfo['PA'], dbInfo['TA']
                               , dbInfo['TD'], dbInfo['WD'], dbInfo['WS'], dbInfo['SZA'], dbInfo['AZA'],
                               dbInfo['ET'], dbInfo['SWR'], dbInfo['ML2'], dbInfo['DL2'], dbInfo['MOD_DATE']
                               , dbInfo['SRV'], dbInfo['ANA_DATE'], dbInfo['DATE_TIME'])
                )
                ...
                session.execute(
                    """
                    INSERT INTO `{}` (SRV, ANA_DATE, DATE_TIME, DATE_TIME_KST, CA_TOT, HM, PA, TA, TD, WD, WS, SZA, AZA, ET, SWR, ML2, DL2, REG_DATE)
                    VALUES ('{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}')
                    """.format(selDbTable
                               , dbInfo['SRV'], dbInfo['ANA_DATE'], dbInfo['DATE_TIME'], dbInfo['DATE_TIME_KST'],
                               dbInfo['CA_TOT'], dbInfo['HM'], dbInfo['PA'], dbInfo['TA']
                               , dbInfo['TD'], dbInfo['WD'], dbInfo['WS'], dbInfo['SZA'], dbInfo['AZA'],
                               dbInfo['ET'],
                               dbInfo['SWR'], dbInfo['ML2'], dbInfo['DL2'], dbInfo['REG_DATE'])
                )
            session.commit()
...
class DtaProcess(object):
    ...
    def exec(self):
        ...
        try:
            ...
            #dtDateList = pd.date_range(start='2022-05-01', end='2022-06-01', freq=Day(1))
            dtDateList = pd.date_range(start='2022-05-20', end='2022-06-01', freq=Day(1))
            #dtDateList = pd.date_range(start='2022-05-01', end='2022-06-01', freq=Day(1))
            ...
            for i, dtDateInfo in enumerate(dtDateList):
                ...
                dtEndDate = pd.to_datetime(dtDateInfo, format='%Y-%m-%d')
                ...
                for idx, posInfo in posDataL1.iterrows():
                    ...
                    tmpData = result['inpDataL1']

                    inpDataL1 = tmpData.loc[
                        (dtIncDateList[0] <= tmpData['ANA_DATE'])
                        & (tmpData['ANA_DATE'] < dtIncDateList[len(dtIncDateList) - 1])
                        ]
                    ...
                    yCol = 'PV'
                    modelKey = 'AI-FOR-20220521-{}'.format(dtDateInfo.strftime("%Y%m%d"))
