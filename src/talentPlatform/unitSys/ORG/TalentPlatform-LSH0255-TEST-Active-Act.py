...
def subActVisPrd(srvId, prdData):
    ...
    try:
        ...
        prdDataL1 = prdData.dropna()

        for key, val in keyList.items():
            log.info("[CHECK] val : {}".format(val))

            mainTitle = '{}_{}_[{}-{}] 기상 실황 정보 (지상 관측소, 위성)를 활용한 머신러닝 ({}) 산점도'.format(srvId, key, minData, maxData, val)
            saveImg = '{}/{}.png'.format(globalVar['figPath'], mainTitle)
            os.makedirs(os.path.dirname(saveImg), exist_ok=True)
            rtnInfo = makeUserHist2dPlot(prdDataL1[key], prdDataL1['PV'], val, '실측', mainTitle, saveImg, 0, 1000, 20, 60, 60, True)
            log.info('[CHECK] rtnInfo : {}'.format(rtnInfo))
        #                                                                                               val)
        #     # # saveImg = '{}/{}/{}/{}.png'.format(globalVar['figPath'], key, srvId, mainTitle)
        #     # saveImg = '{}/{}.png'.format(globalVar['figPath'], mainTitle)
        #     # os.makedirs(os.path.dirname(saveImg), exist_ok=True)
        #     # rtnInfo = makeUserHist2dPlot(actDataL2[key], actDataL2['PV'], val, '실측', mainTitle, saveImg, 0, 1000, 20,
        #     #                              60, 60, True)
        #     # log.info('[CHECK] rtnInfo : {}'.format(rtnInfo))
        #
        #
    ...
    def exec(self):
        ...
        try:
            ...
            if (platform.system() == 'Windows'):
                ...
                globalVar['figPath'] = 'E:/DATA/OUTPUT/AI-FIG'
            ...
            for idx, posInfo in posDataL1.iterrows():
                ...
                # #prdData['DL'] = dlModel.predict(h2o.H2OFrame(prdData[xCol])).as_data_frame()
                ...
                # prdData['DL2'] = dlModel.predict(h2o.H2OFrame(prdData[xCol].dropna())).as_data_frame()
