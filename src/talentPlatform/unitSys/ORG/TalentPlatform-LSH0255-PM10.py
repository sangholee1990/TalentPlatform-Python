    ...
    def exec(self):
        ...
        try:
            ...
            for i, dtIncDateInfo in enumerate(dtIncDateList):
                ...
                saveFile = '{}/TEST/OBS/PM10_OBS_{}_{}.nc'.format(globalVar['outPath'], pd.to_datetime(dtSrtDate).strftime('%Y%m%d'), pd.to_datetime(dtEndDate).strftime('%Y%m%d'))
                ...
                # /SYSTEMS/OUTPUT/OBS/202109/01/AWS_OBS_202109010000.txt
                inpFilePattern = 'OBS/{}/{}/PM10_OBS_{}.txt'.format(dtDateYm, dtDateDay, dtDateYmdHm)
                ...
                for fileInfo in fileList:
                    data = pd.read_csv(fileInfo, header=None, delimiter=',')
                    dataL1 = dataL1.append(data)

                # dataL1 = dataL1.reset_index(drop=True)
                dataL1.columns = ['TM', 'STN', 'PM10', 'tmp', 'tmp', 'tmp']
                dataL2 = dataL1[['TM', 'STN', 'PM10']]
                ...
                for i, dtDateInfo in enumerate(dtDateList):
                    ...
                    # 5분 간격
                    getMin = int(pd.to_datetime(dtDateInfo).strftime('%M'))
                    if (getMin % 5 != 0): continue
                    ...
                    try:
                        dsData = xr.Dataset(
                            {
                                'PM10': (('time', 'lat', 'lon'), varList['PM10'].reshape(1, len(lat1D), len(lon1D)))
                            }
                            , coords={
            ...
            # plt.contourf(dsDataL1['lon'], dsDataL1['lat'], dsDataL1['PM10'][0, :, :])
            # plt.contourf(dsDataL1['lon'], dsDataL1['lat'], dsDataL1['PM10'][0, :, :], levels=100)
            # plt.colorbar()
            # plt.show()
