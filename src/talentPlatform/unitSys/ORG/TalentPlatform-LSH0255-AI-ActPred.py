        ...
        try:
            ...
            for k, modelDirKey in enumerate(modelDirKeyList):
                ...
                for i, posInfo in posDataL1.iterrows():
                    ...
                    mlModelPred = predict_model(mlModel, data=inpDataL1).rename({'Label': 'ML'}, axis='columns')[['dtDateKst', 'ML']]
                    ...
                    tmpData = inpDataL1[['dtDateKst', 'CA_TOT', 'HM', 'PA', 'TA', 'TD', 'WD', 'WS', 'SWR', 'pv', 'sza', 'aza', 'et']].dropna().reset_index(drop=True)
                    dlModelPred = dlModel.predict(h2o.H2OFrame(tmpData)).as_data_frame().rename({'predict': 'DL'}, axis='columns')
                    dlModelPredL1 = pd.concat([tmpData['dtDateKst'], dlModelPred], axis=1)

                    inpDataL2 = tmpData.merge(mlModelPred, how='left', left_on='dtDateKst', right_on='dtDateKst')\
                        .merge(dlModelPredL1, how='left', left_on='dtDateKst', right_on='dtDateKst')

                    # dtDateKst을 기준으로 중복 제거
                    inpDataL2.drop_duplicates(subset=['dtDateKst'], inplace=True)
                    inpDataL2 = inpDataL2.reset_index(drop=True)

                    saveXlsxFile = '{}/{}/{}-SRV{:05d}-{}-{}-{}.xlsx'.format(globalVar['outPath'], 'ACT', serviceName, posId, 'final', 'pred', 'act')
                    ...
                    # **********************************************************************************************************
                    # 폴더 삭제
                    delFile = '{}/{}/{}/SRV{:05d}'.format(globalVar['figPath'], serviceName, figActDirKey, posId)
                    shutil.rmtree(delFile, ignore_errors=True)

                    # idx = inpDataL2.loc[inpDataL2['dtDateKst'] >= pd.to_datetime('2021-01-01', format='%Y-%m-%d')].index.to_numpy()
                    idxInfo = inpDataL2.loc[inpDataL2['dtDateKst'] >= pd.to_datetime('2021-12-01', format='%Y-%m-%d')].index.to_numpy()
                    ...
                    dtDateList = testDataL1['dtDateKst'].dt.strftime('%Y%m%d').unique()
                    # dtDateInfo = dtDateList[199]
                    for j, dtDateInfo in enumerate(dtDateList):

                        testDataL2 = testDataL1.loc[
                            testDataL1['dtDateKst'].dt.strftime('%Y%m%d') == dtDateInfo
                            ]

                        mainTitle = '[{}] {}'.format(dtDateInfo, '기상 실황 정보 (지상 관측소, 위성)를 활용한 예측 시계열')
                        saveImg = '{}/{}/{}/SRV{:05d}/{}.png'.format(globalVar['figPath'], serviceName, figActDirKey, posId, mainTitle)

                        if (os.path.exists(saveImg)): continue

                        os.makedirs(os.path.dirname(saveImg), exist_ok=True)
                        # rtnInfo = makeUserTimeSeriesPlot(testDataL2['dtDate'], testDataL2['ML'], testDataL2['DL'], testDataL2['pv'], '예측 (머신러닝)', '예측 (딥러닝)', '실측 (발전량)', '시간 (시)', '발전량', mainTitle, saveImg, False)
                    ...
                    mainTitle = '[{}-{}] {}'.format(min(dtDateList), max(dtDateList), '기상 실황 정보 (지상 관측소, 위성)를 활용한 머신러닝 (예측) 2D 산점도')
                    saveImg = '{}/{}/{}/SRV{:05d}/{}.png'.format(globalVar['figPath'], serviceName, figActDirKey, posId, mainTitle)
                    os.makedirs(os.path.dirname(saveImg), exist_ok=True)
                    rtnInfo = makeUserHist2DPlot(testDataL1['ML'], testDataL1['pv'], '머신러닝', '실측', mainTitle, saveImg, 0, 1000, 20, 60, 40, True)
                    log.info('[CHECK] rtnInfo : {}'.format(rtnInfo))

                    # mainTitle = '[{}-{}] {}'.format(min(dtDateList), max(dtDateList), '기상 실황 정보 (지상 관측소, 위성)를 활용한 딥러닝 (예측) 2D 산점도')
                    # saveImg = '{}/{}/{}/SRV{:05d}/{}.png'.format(globalVar['figPath'], serviceName, figActDirKey, posId, mainTitle)
                    # os.makedirs(os.path.dirname(saveImg), exist_ok=True)
                    # rtnInfo = makeUserHist2DPlot(testDataL1['DL'], testDataL1['pv'], '딥러닝', '실측', mainTitle, saveImg, 0, 1000, 20, 60, 40, True)
                    # log.info('[CHECK] rtnInfo : {}'.format(rtnInfo))
