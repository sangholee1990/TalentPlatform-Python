...
def setPrdActDataDB(cfgInfo, dbData):
    ...
    def exec(self):
        ...
        try:
            ...
            if (platform.system() == 'Windows'):
                ...
                globalVar['figPath'] = 'E:/DATA/OUTPUT/PV'
            ...
            for idx, posInfo in posDataL1.iterrows():
                ...
                forRes = subForModelInpData(sysOpt, cfgInfo, srvId, posVol, dtIncDateList)
                forInpDataL1 = forRes['inpDataL1']
                forData = forRes['prdData']
                ...
                #     # if (len(forDataL1) < 1): continue
                #
                #     break

                # ===============================================================================
                # [기상실황] 맑은날 전천 (일사량) + AWS/ASOS 융합 지상관측소 (온도)
                # ===============================================================================
                actDataL1 = actData
                solPosInfo = pvlib.solarposition.get_solarposition(
                    pd.to_datetime(actDataL1['DATE_TIME'].values)
                    , posLat
                    , posLon
                    , pressure=actDataL1['PA'].values * 100.0
                    , temperature=actDataL1['TA'].values
                    , method='nrel_numpy'
                )

                actDataL1['SZA'] = solPosInfo['zenith'].values
                actDataL1['ELE'] = solPosInfo['elevation'].values
                actDataL1['AZA'] = solPosInfo['azimuth'].values
                actDataL1['ET'] = solPosInfo['equation_of_time'].values

                dtor = np.pi / 180.0

                # 법선면 일사량
                actDataL1['KMA_SWR'] = 910 * np.sin(actDataL1['ELE'] * dtor) + 0.25 * (910 * np.sin((2 * actDataL1['ELE']) * dtor))
                IZA = (np.sin(actDataL1['SZA'] * dtor) * np.cos((posAza - actDataL1['AZA']) * dtor) * np.sin(posSza * dtor)) + (np.cos(actDataL1['SZA'] * dtor) * np.cos(posSza * dtor))

                STC = -0.415 / 100
                NOCT = actDataL1['TA'] + 1 / 32

                actDataL1['PV_KMA'] = (posVol / 1000) * (actDataL1['KMA_SWR'] * IZA) * (1 + STC * (NOCT - 25))
                actDataL1['PV_H8'] = (posVol / 1000) * (actDataL1['SWR'] * IZA) * (1 + STC * (NOCT - 25))
                actDataL1['PV_DSR'] = (posVol / 1000) * (actDataL1['DSR'] * IZA) * (1 + STC * (NOCT - 25))
                actDataL1['PV_GHI'] = (posVol / 1000) * (actDataL1['GHI_CLR'] * IZA) * (1 + STC * (NOCT - 25))
                actDataL1['PV_POA'] = (posVol / 1000) * (actDataL1['GHI_POA'] * IZA) * (1 + STC * (NOCT - 25))


                # plt.plot(actDataL1['DATE_TIME_KST'], actDataL1['PV_KMA'], label='예측 KMA', marker='o')
                # plt.plot(actDataL1['DATE_TIME_KST'], actDataL1['PV_H8'], label='예측 H8', marker='o')
                # plt.plot(actDataL1['DATE_TIME_KST'], actDataL1['PV_DSR'], label='예측 GK2A', marker='o')
                # plt.plot(actDataL1['DATE_TIME_KST'], actDataL1['PV_GHI'], label='예측 GHI', marker='o')
                # plt.plot(actDataL1['DATE_TIME_KST'], actDataL1['PV_POA'], label='예측 POA', marker='o')
                # plt.plot(actDataL1['DATE_TIME_KST'], actDataL1['PV'], label='실측', marker='o')
                # plt.legend()
                # plt.show()

                actDataL2 = actDataL1.dropna()

                minData = pd.to_datetime(actDataL2['DATE_TIME_KST']).min().strftime("%Y%m%d")
                maxData = pd.to_datetime(actDataL2['DATE_TIME_KST']).max().strftime("%Y%m%d")
                ...
                for key, val in keyList.items():
                    log.info("[CHECK] val : {}".format(val))

                    mainTitle = '{}_{}_[{}-{}] 기상실황 (일사량 및 AWS ASOS 융합 지상관측소 온도)을 활용한 발전량 ({}) 2D 산점도'.format(srvId, key, minData, maxData, val)
                    # saveImg = '{}/{}/{}/{}.png'.format(globalVar['figPath'], key, srvId, mainTitle)
                    saveImg = '{}/{}.png'.format(globalVar['figPath'],  mainTitle)
                    os.makedirs(os.path.dirname(saveImg), exist_ok=True)
                    rtnInfo = makeUserHist2dPlot(actDataL2[key], actDataL2['PV'], val, '실측', mainTitle, saveImg, 0, 1000, 20, 60, 60, True)
                    log.info('[CHECK] rtnInfo : {}'.format(rtnInfo))

                # ===============================================================================
                # [기상예보] 맑은날 전천 (일사량) + UM LDAPS 수치예측모델 (온도)
                # ===============================================================================
                forDataL1 = forData
                solPosInfo = pvlib.solarposition.get_solarposition(
                    pd.to_datetime(forDataL1['DATE_TIME'].values)
                    , posLat
                    , posLon
                    , pressure=forDataL1['PA'].values * 100.0
                    , temperature=forDataL1['TA'].values
                    , method='nrel_numpy'
                )

                forDataL1['SZA'] = solPosInfo['zenith'].values
                forDataL1['ELE'] = solPosInfo['elevation'].values
                forDataL1['AZA'] = solPosInfo['azimuth'].values
                forDataL1['ET'] = solPosInfo['equation_of_time'].values

                dtor = np.pi / 180.0

                # 법선면 일사량
                forDataL1['KMA_SWR'] = 910 * np.sin(forDataL1['ELE'] * dtor) + 0.25 * (910 * np.sin((2 * forDataL1['ELE']) * dtor))
                IZA = (np.sin(forDataL1['SZA'] * dtor) * np.cos((posAza - forDataL1['AZA']) * dtor) * np.sin(posSza * dtor)) + (np.cos(forDataL1['SZA'] * dtor) * np.cos(posSza * dtor))

                STC = -0.415 / 100
                NOCT = forDataL1['TA'] + 1 / 32

                forDataL1['PV_KMA'] = (posVol / 1000) * (forDataL1['KMA_SWR'] * IZA) * (1 + STC * (NOCT - 25))
                forDataL1['PV_UM'] = (posVol / 1000) * (forDataL1['SWR'] * IZA) * (1 + STC * (NOCT - 25))
                forDataL1['PV_GHI'] = (posVol / 1000) * (forDataL1['GHI_CLR'] * IZA) * (1 + STC * (NOCT - 25))
                forDataL1['PV_POA'] = (posVol / 1000) * (forDataL1['GHI_POA'] * IZA) * (1 + STC * (NOCT - 25))
                ...
                minData = pd.to_datetime(forDataL2['DATE_TIME_KST']).min().strftime("%Y%m%d")
                maxData = pd.to_datetime(forDataL2['DATE_TIME_KST']).max().strftime("%Y%m%d")
                ...
                for key, val in keyList.items():
                    ...
                    mainTitle = '{}_{}_[{}-{}] 기상예보 (일사량 및 UM LDAPS 수치예측모델 온도)을 활용한 발전량 ({}) 2D 산점도'.format(srvId, key, minData, maxData, val)
                    # saveImg = '{}/{}/{}/{}.png'.format(globalVar['figPath'], key, srvId, mainTitle)
                    saveImg = '{}/{}.png'.format(globalVar['figPath'], mainTitle)
                    os.makedirs(os.path.dirname(saveImg), exist_ok=True)
                    rtnInfo = makeUserHist2dPlot(forDataL2[key], forDataL2['PV'], val, '실측', mainTitle, saveImg, 0, 1000, 20, 60, 60, True)
                    log.info('[CHECK] rtnInfo : {}'.format(rtnInfo))
